    ...
    try {
      ...
      if (frontImage.value.isNotEmpty && Constant().hasValidUrl(frontImage.value) == false) {
        ...
        frontImage.value = await Constant.uploadUserImageToFireStorage(
            File(frontImage.value), "driverDocument/${FireStoreUtils.getCurrentUid()}", frontImageFileName);
      }
      print('erterrrrrrr4');
      if (backImage.value.isNotEmpty && Constant().hasValidUrl(backImage.value) == false) {
        backImage.value = await Constant.uploadUserImageToFireStorage(
            File(backImage.value), "driverDocument/${FireStoreUtils.getCurrentUid()}", backImageFileName);
      }
      print('erterrrrrrr5');
      ...
      // Upload the document
      bool uploadSuccess = await FireStoreUtils.uploadDriverDocument(documents.value);
      ...
      if (uploadSuccess) {
        ...
        ShowToastDialog.closeLoader();  // Close loader if upload fails
        ShowToastDialog.showToast("Failed to upload the document. Please try again.");
      }
    } catch (e) {
      ShowToastDialog.closeLoader();  // Ensure the loader is closed on error
